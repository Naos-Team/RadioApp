...
export const playSingleSong = (radio) => {
...
export function playWithSelectedSong(selected_radio, radios) {

    let filter_radio = radios.filter(song => {
        return song.song_id !== selected_radio.song_id
    })

    filter_radio.splice(0, 0, selected_radio)

    const tracks = []

    filter_radio.map(radio => {
